...
function Home_Banner_Second() {
    ...
    console.log('teacher data', mission_vission_object)
    ...
    useEffect(() => {
        ...
        if (mission_vission_object.length > 0 && id) {
            const found = mission_vission_object.find(data => data._id === id)
            setSingleData(found)
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`https://speach-and-langauge-responsive.vercel.app/Mission_vission_object_api?id=${id}`, {
                method: 'PUT',
            ...
            if (response.ok) {
                ...
                router.push('/Deshboard/DeshboardHome/Home_Mission_Vission_Ovject_List');
                            ...
                            <div className='py-10 pb-16'>
                                ...
                                <div className='grid grid-cols-2 gap-3'>
                                    <div>
                                        ...
                                        <input  defaultValue={singledata?.Description_Title} type="text" name='descriptiontitle' className="input w-full text-white text-lg" placeholder="Description Title" required />
                                    </div>
                                </div>
                                <div>
                            ...
                            { singledata.dynamicDescriptions?.map((section, index) => (
                                <div key={section.id} className='py-10 pb-16'>
                                    ...
                                    <div className='grid grid-cols-2 gap-3'>
                                        <div>
                                            <legend className="text-lg font-semibold pt-5 text-white">Description Title</legend>
                                            <input defaultValue={section?.title} type="text" name={`descriptionTitle_${index}`} className="input w-full text-lg text-white" placeholder="Description Title" required />
                                        </div>
                                    </div>
                                    <div>
                                        <legend className="text-lg font-semibold pt-5 text-white">Description</legend>
                                        <textarea defaultValue={section?.text} className="textarea w-full text-white text-lg" name={`descriptionDescription_${index}`} placeholder="Description" rows={12} required></textarea>
                                    </div>
                                </div>
                            ))}

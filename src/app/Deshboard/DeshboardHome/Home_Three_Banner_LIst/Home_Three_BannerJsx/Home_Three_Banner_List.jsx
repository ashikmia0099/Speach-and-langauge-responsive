        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const res = await fetch(`https://speach-and-langauge-responsive.vercel.app/Three_Banner_api?id=${id}`, {
                        method: "DELETE"
                    })
                    ...
                    if (res.ok) {
                        ...
                        sethome_three_banner_data((prev) => prev.filter((item) => item._id !== id));
                                ...
                                {/* row 1 */}
                                {
                                    home_three_banner_data.map((data, index) => (
                                        <tr key={data._id}>
                                            ...
                                            <th className=' text-white text-[16px]'>{data?.Title_Banner_One || data?.Title_Banner_Two || data?.Title_Banner_Three}</th>
                                          
                                            <td className=' text-white text-[16px]'>
                                                
                                                <img src={data?.Choose_Event_Image_One || data?.Choose_Event_Image_Two || data?.Choose_Event_Image_Three} className='h-40 w-40 rounded-2xl' alt="Doctor Image" />
                                            ...
                                            >
                                                <Link href={`Home_Three_Banner_LIst/${data._id}`}><FaEdit /></Link>
                                            </td>

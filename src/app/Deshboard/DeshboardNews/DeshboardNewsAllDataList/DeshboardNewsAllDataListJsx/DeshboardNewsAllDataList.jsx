            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (res.ok) {
                        ...
                        set_All_news((prev) => prev.filter((item) => item._id !== id));
                    ...
                    <div className="overflow-x-auto">
                        <table className="table table-zebra">
                            ...
                            <tbody>
                                {/* row 1 */}
                                {
                                    all_news.map((data, index) => (
                                        <tr key={data._id}>

                                            <th className=' text-white text-[16px]'>{index + 1}</th>
                                            <td className=' text-white text-[16px]'>
                                                {data.AboutBannerTitle}
                                                </td>
                                            <td className=' text-white text-[16px]'>
                                                <span className=' text-lg pr-2'>{new Date(data.PostTime).getDate()}</span>
                                                <span className=' text-lg'>{new Date(data.PostTime).toLocaleString('en-US', { month: 'short' })}</span>
                                            </td>
                                            <td
                                                
                                                className='text-3xl text-center text-white cursor-pointer '
                                            >
                                                <Link href={`DeshboardNewsAllDataList/${data._id}`}><FaEdit /></Link>
                                            </td>

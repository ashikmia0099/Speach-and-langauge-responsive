...
function Dashboard_Donation_amount_text_List() {
  ...
  console.log("donation amount", donationAmount)
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (res.ok) {
                        ...
                        setdonationAmount((prev) => prev.filter((item) => item._id !== id));
    ...
    return (
        <div>
            <div className='pt-10 '>
                <h1 className='text-4xl font-semibold text-center text-white py-14'> About Card Data List</h1>
                                ...
                                {/* row 1 */}
                                {
                                    donationAmount.map((data, index) => (
                                        <tr key={data._id}>
                                            ...
                                            <td className=' text-white text-[16px]'>{data.Title_Text_One || data.Donated_Amount}</td>
                                            <td
                                                
                                                className='text-3xl text-center text-white cursor-pointer '
                                            >
                                                <Link href={`Dashboard_Donation_amount_text_List/${data._id}`}><FaEdit /></Link>
                                            </td>

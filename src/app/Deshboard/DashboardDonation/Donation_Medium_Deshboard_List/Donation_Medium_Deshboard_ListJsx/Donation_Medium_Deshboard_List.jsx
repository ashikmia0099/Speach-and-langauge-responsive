...
function Donation_Medium_Deshboard_List() {
    ...
    console.log("donation amount", donationMedium)
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    if (res.ok) {
                        ...
                        setdonationMedium((prev) => prev.filter((item) => item._id !== id));
                                ...
                                {/* row 1 */}
                                {
                                    donationMedium.map((data, index) => (
                                        <tr key={data._id}>
                                            ...
                                            <td className=' text-white text-[16px]'>{data.Bank_or_Mfs_Name}</td>
                                            
                                            <td

                                                className='text-3xl text-center text-white cursor-pointer '
                                            >
                                                <Link href={`Donation_Medium_Deshboard_List/${data._id}`}><FaEdit /></Link>
                                            </td>

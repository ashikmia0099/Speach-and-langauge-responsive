...
export default function StudentMember() {
    ...
    useEffect(() => {
        ...
        fetch('/jsonData/student_member.json')
            .then(res => res.json())
            .then(data => (
                setstudentJson(data)
            ))
        ...
        <div className='max-w-[1596px] mx-auto bg-white'>
            <div className='border-b-2'>
                <h1 className={`text-3xl md:text-6xl lg:text-7xl xl:text-8xl font-extrabold capitalize leading-10 md:leading-14 lg:leading-20 xl:leading-24 text-center px-5 md:px-[10%] xl:px-[20%] pt-7 lg:pt-16`}>
                    <AuroraText>Benefits for SSLT Student Members</AuroraText>
                </h1>
                <p className={`px-5 md:px-[10%] xl:px-[15%] text-[16px] md:text-lg xl:text-2xl leading-7 xl:leading-9 text-center py-10 ${montserrat.className}`}>
                    As a student member of the Society of Speech & Language Therapists (SSLT), you gain access to a wide range of benefits designed to support your educational journey and future professional career in communication, hearing, and swallowing healthcare. By joining SSLT, you become part of a network that prioritizes your growth and development within the field.
                </p>
            </div>

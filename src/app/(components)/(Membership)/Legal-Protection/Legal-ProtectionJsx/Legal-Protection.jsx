...
function Legal_Protection() {


    const [legelproction, setlegelproction] = useState([])

    useEffect(() => {

        fetch('/jsonData/legel_protaction.json')
            .then(res => res.json())
            .then(data => (
                setlegelproction(data)
            ))
    }, [])
        ...
        <div className='max-w-[1596px] mx-auto bg-white'>
            <div className='border-b-2'>
                <h1 className={`text-3xl md:text-6xl lg:text-7xl xl:text-8xl font-extrabold capitalize leading-10 md:leading-14 lg:leading-20 xl:leading-24 text-center px-5 md:px-[10%] xl:px-[20%] pt-7 lg:pt-16`}>
                    <AuroraText>Legal Protection Professionals</AuroraText>
                </h1>
                <p className={`px-5 md:px-[10%] xl:px-[15%] text-[16px] md:text-lg xl:text-2xl leading-7 xl:leading-9 text-center py-10 ${montserrat.className}`}>
                    The Society of Speech & Language Therapists (SSLT) is dedicated to safeguarding the rights and interests of its members. As a member, you can trust SSLT to provide comprehensive legal support in matters concerning your professional practice.
                </p>
            </div>
...
export default Legal_Protection